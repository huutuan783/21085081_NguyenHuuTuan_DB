...
export default function LoginScreen() {
  ...
  useEffect(() => {
    if (isSubmitting && !errors.email && !errors.password) {
      axios
        .post("http://localhost:8084/login", values)
        .then(async (res) => {
          if (res.data === "Success") {
            alert("Login Success");
            await AsyncStorage.setItem("userEmail", values.email);
            navigation.navigate("Screen_giaodien");
          } else {
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={{flexDirection:'row',justifyContent:"center",marginTop:15}}>
        ...
        <TouchableOpacity onPress={()=>navigation.navigate('Screen_dangky')}>
            <Text style={{fontSize:16,fontWeight:'bold'}}>Register Now</Text>
        </TouchableOpacity>
      </View>
    </View>
  );
